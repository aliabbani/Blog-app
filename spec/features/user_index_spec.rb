...
RSpec.describe 'User Index features', js: true do
  before(:each) do
    @amine = User.create(name: 'Amine', photo: 'image1', email: 'amine@mail.com', password: 'asdzxc',
                         posts_counter: 0)
    @ali = User.create(name: 'Ali', photo: 'image2', email: 'ali@mail.com', password: 'asdzxc', posts_counter: 0)
    @henry = User.create(name: 'Henry', photo: 'image3', email: 'henry@mail.com', password: 'asdzxc',
                         posts_counter: 0)
  ...
  it 'When I click on a user, I am redirected to that user\'s show page.' do
    click_link('Amine')
    expect(page).to have_current_path user_path(@amine)
  end
end
